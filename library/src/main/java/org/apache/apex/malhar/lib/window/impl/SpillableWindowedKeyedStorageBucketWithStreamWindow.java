...
/**
 * This implementation use one stream window with one bucket.
 *
 * @param <K>
 * @param <V>
 */
public class SpillableWindowedKeyedStorageBucketWithStreamWindow<K, V> implements WindowedKeyedStorage<K, V, Spillable.SpillableByteMap<K, V>>
{
  protected SpillableByteMapImpl<Long, Window> streamToEventWindow;
  ...
  private SpillableWindowedKeyedStorageBucketWithStreamWindow()
  {
    //for kryo
  }
   ...
   * @param serdeValue
   */
  public SpillableWindowedKeyedStorageBucketWithStreamWindow(byte[] identifier, long bucket, Serde<K, Slice> serdeKey, Serde<V, Slice> serdeValue)
  {
...
//  }
  ...
  public Spillable.SpillableByteMap<K, V> createSpillableMapForWindow(Window window)
  {
    ...
    long bucket = (window.getBeginTimestamp() & 0xffffffff) << 32 + (window.getBeginTimestamp() & 0xffffffff);
