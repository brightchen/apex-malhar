...
public class WindowedOperatorBenchmarkApp extends AbstractWindowedOperatorBenchmarkApp<WindowedOperatorBenchmarkApp.WindowedGenerator, WindowedOperatorBenchmarkApp.MyWindowedOperator>
{
  ...
  protected static class WindowedGenerator extends AbstractGenerator
  {
    public final transient DefaultOutputPort<Tuple.TimestampedTuple<Long>> data = new DefaultOutputPort<Tuple.TimestampedTuple<Long>>();

    @Override
    public void emitTuples()
    {
      for (int i = 0; i < emitBatchSize && emitCount < rate; i++) {
        data.emit(new Tuple.TimestampedTuple<Long>(System.currentTimeMillis() - random.nextInt(120000), (long)random.nextInt(100)));
        emitCount++;
      }
    }
  }
