...
public class SystemResourceMonitor
{
  ...
  public static SystemResource[] memoryResources = new SystemResource[] {SystemResource.CommittedVirtualMemorySize,
      SystemResource.FreePhysicalMemorySize, SystemResource.TotalPhysicalMemorySize, SystemResource.TotalSwapSpaceSize,
      SystemResource.FreeSwapSpaceSize};

  public static SystemResource[] cpuResources = new SystemResource[] {SystemResource.ProcessCpuTime,
      SystemResource.SystemCpuLoad, SystemResource.ProcessCpuLoad};

  public static SystemResource[] fileResources = new SystemResource[] {SystemResource.OpenFileDescriptorCount,
      SystemResource.MaxFileDescriptorCount};

  public static SystemResource[] allResources = SystemResource.values();

  protected static Map<Set<SystemResource>, SystemResourceMonitor> resourcesToMonitors = Maps.newHashMap();

  protected static SystemResourceMonitor defaultInstance;
  ...
  protected SystemResource[] systemResources;
  protected OperatingSystemMXBean operatingSystemMXBean = ManagementFactory.getOperatingSystemMXBean();
  protected Map<SystemResource, Number> systemResourceUsage = Maps.newEnumMap(SystemResource.class);
  protected List<GarbageCollectorMXBean> garbageCollectorMXBeans = ManagementFactory.getGarbageCollectorMXBeans();

  protected SystemResourceMonitor()
  {
  }

  protected SystemResourceMonitor(SystemResource[] systemResources)
  {
  ...
  public Map<SystemResource, Number> getSystemResourceUsage()
  {
    ...
    for (SystemResource sr : systemResources) {
      ...
      try {
        ...
        throw new RuntimeException("Can't invoke method: " + getterMethod.getName(), e);
      }
    }
  ...
  protected void formatEntry(Map.Entry<SystemResource, Number> entry, StringBuilder sb)
  {
    sb.append("\n\t").append(entry.getKey()).append(": ").append(NumberFormat.getInstance().format(entry.getValue()));
  }

  protected Method getGetterMethod(SystemResource sr)
  {
    try {
      return operatingSystemMXBean.getClass().getMethod("get" + sr.name(), null);
    } catch (Exception e) {
      throw new RuntimeException("Invalid SystemResource: " + sr);
    }
  }
